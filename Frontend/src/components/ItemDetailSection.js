...
class ItemDetailSection extends Component {
  ...
  render() {
    ...
    if (item) {
      ...
      const roundPrice =
        parseFloat(item.price.amount).toFixed(item.price.decimals) + "";
      ...
      reactElement = (
        <section className="section section--item-detail">
          <article className="item item-detail">
            ...
            <div className="item-detail__data-container">
              ...
              {item.address ? (
                <label
                  className="item-detail__other-data"
        ...
        // Se podria agregar boton de Back
      );
    } else {
      // Template vacío
    ...
    return <DocumentTitle title={pageTitle}>{reactElement}</DocumentTitle>;
  }
}
