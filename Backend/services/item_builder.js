...
const getItemDetail = function (data) {
  return new Promise(function (resolve, reject) {
    Promise.all([getItem(data, true), getAuthor()])
...
const getDescription = function (itemId) {
  return new Promise(function (resolve, reject) {
    const urlApiItemDescription = API_ENDPOINTS.ITEM_DESCRIPTION.replace(
      ":id",
      itemId
    );
    axios.get(urlApiItemDescription).then(function (response) {
...
const getCurrency = function (currencyId) {
  return new Promise(function (resolve, reject) {
    ...
    axios.get(urlApiCurrency).then(function (currencyResponse) {
      resolve({
...
const getItem = function (data, detailed = false) {
  var promise = new Promise(function (resolve, reject) {
    ...
    let neededPromises = [];
    neededPromises.push(
      getCurrency(data.currency_id).then(function (currencyData) {
    ...
    if (!detailed) {
      ...
      if (data.descriptions && data.descriptions.length > 0) {
        neededPromises.push(
          getDescription(data.id).then(function (description) {
